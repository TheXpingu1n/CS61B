...
public class GameOfLife {
    ...
    public TETile[][] nextGeneration(TETile[][] tiles) {
        ...
        for(int x = 0; x < width; x++) {
            for(int y = 0; y < height; y++) {
                int type = cellType(x, y);
                int AliveNeighbours = state(x,y,type,tiles);
                Azrael(x, y, AliveNeighbours, nextGen,tiles);
            }
        }
    ...
    private int cellType(int i, int j)
    {
        if(i-1 < 0 && j-1 < 0)
            return 1;
        else if(j-1 < 0 && i-1 >= 0 && i + 1 < width)
            return 2;
        else if(j-1 < 0 && i + 1 >= width)
            return 3;
        else if(i-1 < 0 && j+1 < height && j-1 >= 0)
            return 4;
        else if(i+1 >= width && j+1 < height && j-1 >= 0)
            return 5;
        else if(i-1 < 0 && j+1 >= height)
            return 6;
        else if(j+1 >= height && i-1 >= 0 && i + 1 < width)
            return 7;
    ...
    public int state(int i, int j, int type, TETile[][] tiles)
    {
        int AliveCount = 0;
        switch (type) {
            case 1:
                if(tiles[i][j+1] == Tileset.CELL)
                    AliveCount++;
                if(tiles[i+1][j+1] == Tileset.CELL)
                    AliveCount++;
                if(tiles[i+1][j] == Tileset.CELL)
                    AliveCount++;
                return AliveCount;
            case 2:
                if(tiles[i-1][j+1] == Tileset.CELL)
    ...
    public void Azrael(int i, int j, int neighbors, TETile[][] newtiles, TETile[][] oldtiles)
    {
        if(neighbors == 3)
            newtiles[i][j] = Tileset.CELL;
        else if(neighbors == 2 && oldtiles[i][j] == Tileset.CELL)
        {
            newtiles[i][j] = Tileset.CELL;
        }
        else
            newtiles[i][j] = Tileset.NOTHING;
    }
    /**
        ...
        for (int i = height-1; i >= 0; i--) {
            for (int j = 0; j < width; j++) {
                if(currentState[j][i] == Tileset.CELL)
                    builder.append("1");
                else if(currentState[j][i] == Tileset.NOTHING)
                    builder.append("0");
            }
            builder.append("\n");
        }
    ...
    public TETile[][] loadBoard(String filename) {
        ...
        String[] res = s.split("\n");
        String[] dimensions = res[0].split(" ");
        // TODO: Grab and set the dimensions from the first line.
        width = Integer.parseInt(dimensions[0]);
        height = Integer.parseInt(String.valueOf(dimensions[1]));
        ...
        for(int i = 0; i < newres.length; i++) {
            for (int k = 0; k < newres[i].length(); k++) {
                if(newres[i].charAt(k) == '1') {
                    newtiles[i][k] = Tileset.CELL;
                }
                else {
                    newtiles[i][k] = Tileset.NOTHING;
                }
            }
        }
