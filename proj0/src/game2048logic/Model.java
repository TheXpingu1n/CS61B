...
public class Model {
        ...
        for (int i = 0; i < board.size(); i++) {
            for (int j = 0; j < board.size(); j++) {
                if(board.tile(i,j) != null)
                {
                    ...
                    if(i-1 > 0 && board.tile(i,j).value() == board.tile(i-1,j).value())
                    {
                        return true;
                    }
                    if(j-1 > 0 && board.tile(i,j).value() == board.tile(i,j-1).value())
                    {
                        return true;
                    }
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
       ...
       while (targetY < board.size()-1)
       {
           ...
           {
               if(myValue == board.tile(x, targetY+1).value() && !currTile.wasMerged() &&
